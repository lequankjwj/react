...
import { IGetInsightHome, IGetService } from '@/models';
import {
  setFooterMenu,
  setListService,
...
const withCommon = ({ callback = () => null }: GsspType) =>
  wrapper.getServerSideProps((store): any => async (ctx: GetServerSidePropsContext) => {
    ...
    const promises = [
      ...
      readCache(CACHE_LIST_SERVICE) ?? commonService.getServiceData(region),
    ...
    const [menuList, footerMenu, listServices, updateSection] = await response.map((item) =>
      item.status === 'fulfilled' ? item.value ?? [] : null,
    );

    store.dispatch(setMenuHeader(menuList));
    store.dispatch(setFooterMenu(footerMenu));
    store.dispatch(setListService(listServices));
